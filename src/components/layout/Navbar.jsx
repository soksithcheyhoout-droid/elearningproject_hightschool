...
export default function Navbar({ activeTab, setActiveTab, onOpenAITutor, onSelectSubject, adminSession, onOpenAdminLogin, onOpenDonation, onToggleSidebar, onOpenYouTube, isYouTubeMusicPlaying }) {
  ...
  return (
    ...
    >
      ...
      {/* 1. Official MoEYS Full-Width Top Utility Ribbon (Edge-to-Edge on PC / Laptops) */}
      <div 
        className={`hidden md:block w-full bg-gradient-to-r from-[#002d62] via-[#005baa] to-[#0077cc] text-white select-none overflow-hidden transition-all duration-300 ease-in-out border-b border-white/15 ${
      ...
      <div className="w-full max-w-[1600px] mx-auto px-2 sm:px-4 lg:px-6 relative z-20">
        <div className={`flex items-center justify-between gap-1 sm:gap-3 min-w-0 transition-all duration-300 ${
          isScrolled ? 'h-[64px]' : 'h-[64px] md:h-[80px]'
        }`}>
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 flex-shrink-0 ml-auto pl-1 sm:pl-2 pr-1.5 sm:pr-3">
            ...
            <div ref={searchContainerRef} className="relative flex-shrink-0">
              <button
                ...
                className={`p-1.5 sm:p-2 rounded-xl transition-all duration-200 flex items-center justify-center cursor-pointer border flex-shrink-0 select-none active:scale-95 ${
                  isSearchOpen
                    ? 'bg-blue-100 dark:bg-slate-800 text-[#005baa] dark:text-cyan-300 border-[#005baa] dark:border-cyan-400 shadow-sm'
                    : 'bg-slate-50 hover:bg-blue-50/80 dark:bg-slate-800/90 dark:hover:bg-slate-700 text-[#005baa] dark:text-cyan-400 hover:text-[#002d62] dark:hover:text-cyan-200 border-slate-200/90 dark:border-slate-700/80 hover:border-[#005baa]/40 dark:hover:border-cyan-400/40 shadow-2xs'
                }`}
                ...
                <>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-100 dark:border-slate-800 sm:hidden">
                      <span className="text-xs font-black text-[#003366] dark:text-white flex items-center gap-1.5">
                        <Search className="w-3.5 h-3.5 text-[#005baa] dark:text-cyan-400" />
                        <span>ស្វែងរកទិន្នន័យ (Search)</span>
                      </span>
                      <button
                        type="button"
                        onClick={handleCloseSearch}
                        className="w-6 h-6 rounded-lg text-slate-400 hover:text-slate-800 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center cursor-pointer transition-colors"
                        title="បិទ (Close)"
                      >
                        <X className="w-4 h-4" />
                      </button>
                    ...
                    <div className="relative mb-3 flex-shrink-0">
                      <input
                        ...
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-2xl pl-10 pr-9 py-2 sm:py-2.5 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 font-bold focus:outline-none focus:border-[#005baa] dark:focus:border-cyan-400 focus:bg-white dark:focus:bg-slate-950 focus:ring-2 focus:ring-[#005baa]/20 dark:focus:ring-cyan-500/20 shadow-xs transition-all"
                        autoFocus
                      />
                      <Search className="w-4 h-4 text-[#005baa] dark:text-cyan-400 absolute left-3.5 top-2.5 sm:top-3 pointer-events-none" />
                      {searchQuery && (
                        <button
                          type="button"
                          onClick={() => setSearchQuery('')}
                          className="p-1 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-800 absolute right-2.5 top-2 cursor-pointer transition-colors"
                        >
                    ...
                    <div className="overflow-y-auto pr-1 flex-1 max-h-[360px] [scrollbar-width:thin]">
                      {searchQuery.trim() === '' ? (
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {['គណិតវិទ្យា', 'បាក់ឌុប', 'រូបវិទ្យា', 'គីមីវិទ្យា', 'ភាសាខ្មែរ', 'ជីវវិទ្យា', 'STEM'].map((tag) => (
                              <button
                                ...
                                className="px-2.5 py-1 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-[11px] font-bold text-slate-700 dark:text-slate-300 hover:text-[#005baa] dark:hover:text-cyan-300 hover:border-blue-300 dark:hover:border-slate-600 transition-all cursor-pointer shadow-2xs hover:scale-105"
                              >
                                {tag}
                              </button>
                            ))}
                        ...
                        <div className="space-y-1.5 pt-0.5">
                          <div className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 px-1 flex items-center justify-between">
                          ...
                          {searchResults.map((item) => (
                            <div
                              key={item.id}
                              onClick={item.action}
                              className="p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 hover:bg-blue-50/80 dark:hover:bg-slate-700/60 border border-slate-200/90 dark:border-slate-700 cursor-pointer transition-all flex items-center justify-between group text-left shadow-2xs"
                            >
                              <div className="flex-1 min-w-0 pr-2">
                                ...
                                <h5 className="text-xs sm:text-[13px] font-black text-[#003366] dark:text-white truncate group-hover:text-[#005baa] dark:group-hover:text-cyan-300 transition-colors">
                                  {item.title}
                                </h5>
                              ...
                              <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-[#005baa] dark:group-hover:text-cyan-300 group-hover:translate-x-1 transition-all flex-shrink-0" />
                            </div>
                          ))}
                        </div>
                      ) : (
            ...
            <div ref={announcementContainerRef} className="relative flex-shrink-0">
              <button
                ...
                className={`relative p-1.5 sm:p-2 rounded-xl transition-all duration-200 flex items-center justify-center cursor-pointer border flex-shrink-0 select-none active:scale-95 ${
                  isAnnouncementOpen
                    ? 'bg-blue-100 dark:bg-slate-800 text-[#005baa] dark:text-cyan-300 border-[#005baa] dark:border-cyan-400 shadow-sm'
                    : 'bg-slate-50 hover:bg-blue-50/80 dark:bg-slate-800/90 dark:hover:bg-slate-700 text-[#005baa] dark:text-cyan-400 hover:text-[#002d62] dark:hover:text-cyan-200 border-slate-200/90 dark:border-slate-700/80 hover:border-[#005baa]/40 dark:hover:border-cyan-400/40 shadow-2xs'
                }`}
                  ...
                  >
                    ...
                    <div className="space-y-2.5 overflow-y-auto pr-1 flex-1 max-h-[380px] [scrollbar-width:thin]">
                      ...
                      <div 
                        ...
                        className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 hover:bg-blue-50/80 dark:hover:bg-slate-700/60 border border-slate-200/90 dark:border-slate-700 hover:border-[#005baa]/50 dark:hover:border-cyan-500/50 cursor-pointer transition-all space-y-1.5 group shadow-2xs text-left"
                      >
                        <div className="flex items-center justify-between text-[10px]">
                          <span className="px-2 py-0.5 rounded-md font-bold bg-rose-100 dark:bg-rose-950/60 text-rose-800 dark:text-rose-300 border border-rose-200 dark:border-rose-800">
                        ...
                        <h5 className="text-xs sm:text-[13px] font-black text-[#003366] dark:text-white leading-snug group-hover:text-[#005baa] dark:group-hover:text-cyan-300 transition-colors">
                        ...
                        <div className="flex items-center justify-between pt-1 border-t border-slate-200/60 dark:border-slate-700/60 text-[10.5px]">
                          <span className="text-slate-400 dark:text-slate-500 text-[10px]">មុននេះ ១ ម៉ោង</span>
                          <div className="flex items-center gap-1 font-bold text-[#005baa] dark:text-cyan-400 group-hover:translate-x-0.5 transition-transform">
                      ...
                      >
                        <div className="flex items-center justify-between text-[10px]">
                          <span className="px-2 py-0.5 rounded-md font-bold bg-cyan-100 dark:bg-cyan-950/60 text-cyan-800 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-800">
                            កម្មវិធីជាតិ • 1v1 Arena
                          </span>
                          <span className="text-cyan-700 dark:text-cyan-300 font-bold bg-cyan-50 dark:bg-cyan-950/40 px-1.5 py-0.2 rounded">
                            រង្វាន់ +500 XP
                          </span>
                        </div>
            ...
            <div className="relative flex-shrink-0">
              <button
                ...
                className={`h-8 sm:h-9 px-2 sm:px-2.5 rounded-xl border text-[11px] sm:text-xs font-black transition-all duration-200 flex items-center gap-1 cursor-pointer flex-shrink-0 select-none active:scale-95 ${
                  isLangOpen
                    ? 'bg-blue-100 dark:bg-slate-800 text-[#005baa] dark:text-cyan-300 border-[#005baa] dark:border-cyan-400 shadow-sm'
                    : 'bg-slate-50 hover:bg-blue-50/80 dark:bg-slate-800/90 dark:hover:bg-slate-700 text-[#005baa] dark:text-cyan-400 hover:text-[#002d62] dark:hover:text-cyan-200 border-slate-200/90 dark:border-slate-700/80 hover:border-[#005baa]/40 dark:hover:border-cyan-400/40 shadow-2xs'
                }`}
              ...
              {isLangOpen && (
                <div className="absolute top-full right-0 mt-1.5 w-32 bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-2xl rounded-2xl border border-white/80 dark:border-slate-800 shadow-2xl p-1.5 z-50 animate-scale-up select-none ring-1 ring-black/5 dark:ring-white/10">
                  <button
                    type="button"
                    ...
                    className={`w-full px-3 py-2 rounded-xl text-xs font-bold text-left flex items-center gap-2 cursor-pointer transition-colors ${
                      lang === 'km' 
                        ? 'bg-[#005baa] dark:bg-cyan-600 text-white shadow-xs' 
                        : 'text-[#003876] dark:text-slate-200 hover:bg-blue-50/80 dark:hover:bg-slate-800 hover:text-[#005baa] dark:hover:text-cyan-300'
                    }`}
                  >
                    <span>🇰🇭</span>
                    <span>ភាសាខ្មែរ</span>
            ...
            <button
              ...
              title={lang === 'km' ? 'ស្វែងរក និងចាក់ចម្រៀង YouTube (តន្ត្រីនៅតែបន្តចាក់ពេលបិទ)' : 'Search & Play YouTube Music (Keeps playing when closed)'}
            >
              <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current flex-shrink-0" viewBox="0 0 24 24">
