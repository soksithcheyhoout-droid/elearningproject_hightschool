...
export default function MinistryDonationModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-2 sm:p-4 md:p-6 bg-slate-950/80 backdrop-blur-md animate-fadeIn font-kantumruy select-none overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-white rounded-2xl sm:rounded-3xl shadow-[0_25px_70px_rgba(0,15,40,0.35)] border border-slate-200 overflow-hidden animate-scaleUp flex flex-col md:flex-row max-h-[92vh] sm:max-h-[94vh] my-auto">
        ...
        <div className="flex-1 flex flex-col justify-between bg-slate-50/90 p-4 sm:p-6 md:p-7 overflow-y-auto relative min-h-0">
          
          {/* Modal Header */}
          <div className="flex items-center justify-between pb-3 border-b border-slate-200">
            <div className="space-y-0.5 pr-8 md:pr-0">
              <span className="text-[9.5px] sm:text-[10px] font-mono uppercase font-black text-red-600 tracking-wider block">
                {step === 1 
              ...
              <h3 className="text-xs sm:text-base font-black text-slate-900 font-kantumruy">
                {step === 1 
            ...
            {/* Desktop-only internal close button (Mobile uses the persistent top-right floating button) */}
            <button
              type="button"
              onClick={onClose}
              className="hidden md:flex w-8 h-8 rounded-full bg-slate-200/80 hover:bg-slate-300 text-slate-600 items-center justify-center transition-colors cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          {isGeneratingQR && (
            <div className="my-auto py-3 flex flex-col items-center justify-center space-y-4 animate-fadeIn">
              ...
              <div className="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-5 shadow-sm">
                ...
                <div className="flex items-center justify-between gap-2 relative">
                  ...
                  <div className="flex flex-col items-center text-center w-24">
                    ...
                    <span className="text-[11.5px] font-mono font-black text-slate-900 mt-2">chey_dev</span>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center px-1 relative">
                    ...
                    <div className="relative w-full h-10 flex items-center justify-center">
                      <svg className="w-full h-6 overflow-visible" preserveAspectRatio="none" viewBox="0 0 100 20">
                        ...
                        <line x1="0" y1="10" x2="100" y2="10" stroke="#e2e8f0" strokeWidth="2.5" strokeLinecap="round" />
                      ...
                      {/* Security Lock Badge */}
                      <div className="absolute z-10 px-2.5 py-0.5 rounded-full bg-white border border-slate-300 shadow-xs flex items-center gap-1">
                        <Lock className="w-2.5 h-2.5 text-emerald-600" />
                        <span className="text-[9px] font-mono font-bold text-slate-700">TLS 1.3 / EMVCo</span>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col items-center text-center w-24">
                    ...
                    <span className={`text-[11.5px] font-mono font-bold mt-2 ${
                      paymentGateway === 'aba' ? 'text-[#004B87]' : 'text-red-600'
                    }`}>
              ...
              <div className="w-full max-w-md bg-white border border-slate-200 rounded-2xl p-3.5 space-y-2.5 text-left shadow-xs">
                
                {/* Step 1 */}
                <div className="flex items-center gap-2.5 text-xs">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                    genStage >= 1 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'
                  }`}>
                    {genStage > 1 ? '✓' : <Loader2 className="w-3 h-3 animate-spin text-emerald-600" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="font-bold text-slate-900 block leading-none text-[11.5px]">
                ...
                <div className="flex items-center gap-2.5 text-xs">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                    genStage >= 2 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'
                  }`}>
                    {genStage > 2 ? '✓' : genStage === 2 ? <Loader2 className="w-3 h-3 animate-spin text-blue-600" /> : '2'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="font-bold text-slate-900 block leading-none text-[11.5px]">
                ...
                <div className="flex items-center gap-2.5 text-xs">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                    genStage >= 3 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'
                  }`}>
                    {genStage === 3 ? <Loader2 className="w-3 h-3 animate-spin text-red-600" /> : '3'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="font-bold text-slate-900 block leading-none text-[11.5px]">
              ...
              <div className="text-center">
                <span className="text-[10.5px] font-mono text-slate-500 bg-slate-100 px-3.5 py-1 rounded-full border border-slate-200">
                  Gateway: <strong className="text-slate-900 uppercase">{paymentGateway === 'aba' ? 'ABA PayWay' : 'Bakong NBC'}</strong> • Dev: <strong className="text-slate-800">@chey_dev</strong> • Currency: <strong className="text-red-600">{amountType.toUpperCase()}</strong>
                </span>
              </div>
          ...
          {step === 1 && !isGeneratingQR && (
            <div className="space-y-3.5 py-2 animate-fadeIn">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-2xl border text-left transition-all cursor-pointer relative overflow-hidden flex items-center gap-2.5 ${
                      paymentGateway === 'aba'
                        ? 'bg-[#002D56]/5 border-[#004B87] shadow-xs ring-2 ring-[#00A3E0]/40'
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1">
                        <span className="font-black text-xs text-slate-900">ABA PayWay</span>
                        <span className="text-[8px] px-1 py-0.2 rounded bg-[#00A3E0]/15 text-[#004B87] font-bold">1-Click</span>
                      </div>
                      <span className="text-[9.5px] text-slate-500 block truncate">Direct ABA Mobile</span>
                    </div>
                    {paymentGateway === 'aba' && (
                      <div className="w-4 h-4 rounded-full bg-[#004B87] text-white flex items-center justify-center text-[9px] font-bold">✓</div>
                    )}
                  </button>
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-2xl border text-left transition-all cursor-pointer relative overflow-hidden flex items-center gap-2.5 ${
                      paymentGateway === 'bakong'
                        ? 'bg-red-500/5 border-red-500 shadow-xs ring-2 ring-red-400/40'
                        : 'bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50'
                    }`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1">
                        <span className="font-black text-xs text-slate-900">Bakong KHQR</span>
                        <span className="text-[8px] px-1 py-0.2 rounded bg-red-100 text-red-700 font-bold">All Banks</span>
                      </div>
                      <span className="text-[9.5px] text-slate-500 block truncate">National NBC System</span>
                    </div>
                    {paymentGateway === 'bakong' && (
                      <div className="w-4 h-4 rounded-full bg-red-600 text-white flex items-center justify-center text-[9px] font-bold">✓</div>
                    )}
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center p-1 bg-slate-200/70 rounded-2xl">
                ...
                >
                  <span className="font-mono font-bold text-red-600 text-[11px]">KHR</span>
                    ...
                    return (
                      <button
                        ...
                        className={`p-2.5 rounded-2xl border text-center transition-all cursor-pointer relative overflow-hidden flex flex-col items-center justify-center min-h-[64px] ${
                          isSelected
                            ? paymentGateway === 'aba'
                              ? 'bg-[#002D56]/10 border-[#004B87] text-[#002D56] shadow-xs ring-2 ring-[#00A3E0]/40'
                              : 'bg-red-50/90 border-red-500 text-red-600 shadow-xs ring-2 ring-red-400'
                            : 'bg-white border-slate-200 text-slate-800 hover:border-slate-300 hover:bg-slate-50'
                        }`}
                      >
                        {tier.popular && (
                          <span className={`absolute top-0 right-0 text-white font-black text-[7.5px] px-1.5 py-0.2 rounded-bl-md uppercase tracking-wider ${
                            paymentGateway === 'aba' ? 'bg-[#004B87]' : 'bg-red-600'
                          }`}>
                            Popular
                          </span>
                        )}
                        ...
                        <span className="text-[9.5px] font-bold mt-1 leading-none text-slate-700">
                          {tier.title}
                        </span>
                        <span className="text-[8.5px] text-slate-400 font-mono mt-0.5 leading-none">
                          {tier.note}
                        </span>
                      </button>
                    );
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-3.5 pr-16 py-2 rounded-xl border border-slate-200 bg-white focus:outline-none focus:border-slate-900 text-xs font-bold text-slate-800 transition-all font-mono"
                  />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div>
                  <label className="text-[10.5px] font-bold text-slate-600 block mb-0.5 flex items-center gap-1">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-800 focus:outline-none focus:border-slate-900"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 bg-white text-xs text-slate-800 focus:outline-none focus:border-slate-900"
            ...
            <div className="space-y-3 py-1 flex flex-col items-center justify-center animate-fadeIn">
              ...
              <div className="flex items-center justify-between gap-2.5 w-full max-w-sm pt-1">
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="flex-1 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold cursor-pointer transition-colors text-center"
                >
                  ថយក្រោយ (Back)
                </button>
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer transition-colors shadow-sm"
                >
          ...
          {step === 3 && !isGeneratingQR && (
            <div className="space-y-2.5 py-1 text-center animate-scaleUp max-w-md mx-auto w-full">
              ...
              <div className="flex flex-col items-center justify-center">
                <div className="relative w-12 h-12 rounded-2xl bg-white border-2 border-amber-400/90 shadow-md p-1.5 flex items-center justify-center ring-4 ring-amber-100">
                  <img 
                    src="/assets/moeys-crest-transparent.png" 
                    alt="Ministry Crest" 
                    className="w-full h-full object-contain filter drop-shadow-xs"
                    onError={(e) => { e.currentTarget.src = '/assets/moeys-custom-logo-transparent.png'; }}
                  />
                  <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-emerald-500 text-white flex items-center justify-center text-[9px] font-bold ring-1.5 ring-white">
                    ✓
                  </div>
                </div>
                <h4 className="font-black text-base text-slate-900 mt-1.5 flex items-center justify-center gap-1">
                  <span>ការទូទាត់ទទួលបានជោគជ័យ!</span>
                </h4>
                <p className="text-[11px] text-slate-500">
                  សូមថ្លែងអំណរគុណយ៉ាងជ្រាលជ្រៅចំពោះសប្បុរសជន <strong className="text-slate-800 font-bold">{donorName}</strong>
                </p>
              </div>

              {/* Official Ministry Watermarked Receipt Voucher */}
              <div className="relative overflow-hidden bg-white border border-slate-200/90 rounded-2xl p-3.5 text-left shadow-xs space-y-2 text-xs">
                {/* Official Ministry Crest Watermark Background */}
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none opacity-[0.14] overflow-hidden">
                  <img 
                    src="/assets/moeys-crest-transparent.png" 
                ...
                <div className="relative z-10 flex items-center justify-between pb-2 border-b border-dashed border-slate-200">
                  <div className="flex items-center gap-2">
                    <img 
                      src="/assets/moeys-crest-transparent.png" 
                      alt="Crest" 
                    ...
                    />
                    <div>
                      <p className="font-black text-[10.5px] sm:text-[11px] text-slate-900 leading-tight">
                        ក្រសួងអភិវឌ្ឍន៍ទេពកោសល្យ និងការស្រាវជ្រាវកម្រិតខ្ពស់
                      </p>
                      <p className="text-[8px] sm:text-[8.5px] text-slate-500 font-mono uppercase tracking-wider">
                        MINISTRY OF TALENT DEVELOPMENT & ADVANCED RESEARCH
                      </p>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <span className="text-[9px] font-mono font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded border border-slate-200/80">
                ...
                <div className="relative z-10 space-y-1.5 text-[11px]">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">សប្បុរសជន (Benefactor)</span>
                    <span className="font-bold text-slate-900">{donorName}</span>
                  </div>
                  
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">ចំនួនទឹកប្រាក់ (Contribution)</span>
                    <span className="font-mono font-black text-emerald-600 text-sm">{displayAmount}</span>
                  </div>

                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">ច្រកទូទាត់ (Payment Gateway)</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">កាលបរិច្ឆេទ (Date & Time)</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500">លេខកូដយោង (Transaction Ref)</span>
                    <span className="font-mono text-[9.5px] text-slate-700 bg-slate-50 px-1.5 py-0.5 rounded border border-slate-200">
                      {khqrData?.md5 ? `MD5-${khqrData.md5.slice(0, 12).toUpperCase()}` : `TXN-${Date.now().toString().slice(-8)}`}
                    </span>
                  </div>

                  {donorMessage && (
                    <div className="pt-1 border-t border-dashed border-slate-200 text-slate-600 italic bg-amber-50/50 p-1.5 rounded border border-amber-200/60">
                      "{donorMessage}"
                    </div>
                  )}
                </div>
                ...
                <div className="relative z-10 flex items-center justify-between pt-2 border-t border-dashed border-slate-200 text-[9px] text-slate-500">
              ...
              <div className="flex items-center justify-center gap-1.5 text-[11px] text-emerald-800 bg-emerald-50/90 border border-emerald-200/90 rounded-xl py-1.5 px-3 font-bold shadow-2xs">
              ...
              <button
                type="button"
                onClick={onClose}
                className="w-full py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs transition-all shadow-sm cursor-pointer active:scale-[0.99]"
              >
                រួចរាល់ (Done)
              </button>
