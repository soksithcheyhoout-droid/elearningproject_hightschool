...
export default function SubjectGrid({ onSelectSubject, showHeroBanner = true }) {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 font-kantumruy">
      ...
      {/* Official Curriculum Catalog Header */}
      {showHeroBanner && (
        <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden bg-[#00224d] border border-white/15 shadow-lg text-white">
          
          {/* Subtle Background Pattern */}
          <div className="absolute inset-0 bg-gradient-to-r from-[#001736]/90 to-[#002d62]/80 pointer-events-none" />

          {/* National Crest Watermark */}
          <div className="absolute right-2 sm:right-12 top-1/2 -translate-y-1/2 w-48 sm:w-72 h-48 sm:h-72 pointer-events-none select-none opacity-10 mix-blend-screen z-0">
            <img
          ...
          <div className="relative z-10 p-4 sm:p-7 lg:p-8 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-6 sm:gap-8">
            ...
            <div className="space-y-3 sm:space-y-4 max-w-2xl min-w-0">
              ...
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 rounded-lg bg-white/10 text-amber-300 font-bold text-[11px] sm:text-xs flex items-center gap-1.5 border border-white/15 shrink-0">
                  <ShieldCheck className="w-3.5 h-3.5 text-amber-300" />
                  <span>{lang === 'km' ? 'កម្មវិធីសិក្សាជាតិផ្លូវការ' : 'Official National Curriculum'}</span>
                </span>
                
                <span className="px-2.5 py-1 rounded-lg bg-white/5 text-slate-300 text-[11px] font-medium border border-white/10 hidden sm:inline-flex">
                  {lang === 'km' ? 'ថ្នាក់ទី ១ ដល់ ទី ១២' : 'Grades 1 to 12'}
                </span>
              </div>
              ...
              <div className="relative w-full max-w-md pt-0.5">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-8 py-2.5 rounded-xl bg-white/95 focus:bg-white text-slate-900 placeholder-slate-400 text-xs font-semibold shadow-sm focus:outline-none focus:ring-2 focus:ring-amber-400 transition-all"
                />
                {searchQuery && (
                  <button
            ...
            <div className="w-full lg:w-72 bg-slate-950/50 rounded-2xl p-4 border border-white/10 space-y-3 shadow-md shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-xl bg-white/10 flex items-center justify-center p-1.5 border border-white/15 shrink-0">
                  <img
                    src="/assets/moeys-crest-transparent.png"
                    alt="MoTDAR Crest"
                    className="w-full h-full object-contain filter brightness-125"
                ...
                <div className="min-w-0">
                  <h4 className="text-[11px] font-bold text-amber-300 uppercase tracking-wide truncate">
              ...
              <div className="grid grid-cols-2 gap-2 text-center">
                <div className="bg-white/5 rounded-xl p-2 border border-white/10">
                  <span className="text-[9px] text-slate-300 block font-bold">{lang === 'km' ? 'វិទ្យាសាស្ត្រពិត' : 'Science'}</span>
                  <span className="text-xs sm:text-sm font-black text-sky-400">{totalScienceCount} Subjects</span>
                </div>
                <div className="bg-white/5 rounded-xl p-2 border border-white/10">
                  <span className="text-[9px] text-slate-300 block font-bold">{lang === 'km' ? 'វិទ្យាសាស្ត្រសង្គម' : 'Social'}</span>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2.5 sm:gap-4">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 font-kantumruy text-xs overflow-x-auto pb-1 sm:pb-0 scrollbar-none flex-nowrap w-full md:w-auto">
            <button
              ...
              className={`py-1.5 px-3 sm:px-4 sm:py-2 rounded-xl transition-all font-bold flex items-center justify-center gap-1 text-[11px] sm:text-xs cursor-pointer active:scale-95 shrink-0 ${
                filterStream === 'all'
