...
// Comprehensive Standard Unicode Emoji Categories
...
export default function StudentMessengerView({ onLaunchDuelGame, onBack }) {
    ...
    const doScroll = () => {
      if (chatContainerRef.current) {
        chatContainerRef.current.scrollTo({
          top: chatContainerRef.current.scrollHeight + 9999,
          behavior
        });
        setShowScrollBottom(false);
      }
    };
    // Double-RAF ensures the DOM has painted the new content before scrolling
  ...
  const filteredEmojis = useMemo(() => {
    ...
    if (!q) {
      return EMOJI_CATEGORIES.find(c => c.id === activeEmojiCategory)?.emojis || EMOJI_CATEGORIES[0].emojis;
    }
    const matched = [];
    EMOJI_CATEGORIES.forEach(cat => {
      cat.emojis.forEach(em => {
        if (!matched.includes(em)) matched.push(em);
      });
    });
    return matched;
          ...
          <div className="bg-white border-t border-slate-200 px-1.5 sm:px-3 py-1.5 sm:py-2.5 flex-shrink-0 relative z-40 shadow-xs w-full max-w-full overflow-visible" style={{paddingBottom:'max(6px, env(safe-area-inset-bottom, 6px))'}}>
            <form onSubmit={handleSendMessage} className="flex items-center gap-1 sm:gap-2 relative w-full max-w-full">
              ...
              <div ref={emojiPickerRef} className="relative flex-shrink-0">
                ...
                {/* Emoji Picker Popover - rendered via Portal to avoid clipping */}
                {isEmojiPickerOpen && createPortal(
                  <div 
                    onClick={() => { setIsEmojiPickerOpen(false); setEmojiSearchQuery(''); }}
                    className="fixed inset-0 z-[9999]"
                    style={{background:'transparent'}}
                  >
                    <div 
                      onClick={(e) => e.stopPropagation()}
                      className="fixed z-[10000] w-[360px] max-w-[calc(100vw-16px)] bg-white/98 backdrop-blur-2xl rounded-2xl border border-slate-200 shadow-2xl p-2.5 sm:p-3.5 select-none ring-1 ring-black/10 font-kantumruy drop-shadow-2xl animate-scale-up"
                      style={{
                        bottom: `${window.innerHeight - (emojiPickerRef.current?.getBoundingClientRect()?.top || 0) + 8}px`,
                        left: `${Math.max(8, (emojiPickerRef.current?.getBoundingClientRect()?.left || 0))}px`
                      }}
                    >
                      <div className="flex items-center justify-between pb-2 border-b border-slate-100 gap-2">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="w-5 h-5 rounded-lg bg-amber-100 flex items-center justify-center flex-shrink-0">
                            <Smile className="w-3.5 h-3.5 text-amber-600" />
                          </span>
                          <span className="font-extrabold text-xs text-[#003366] truncate">
                            EMOJI ({filteredEmojis.length})
                          </span>
                        </div>
                        <button 
                          type="button"
                          onClick={() => { setIsEmojiPickerOpen(false); setEmojiSearchQuery(''); }}
                          className="text-slate-400 hover:text-slate-700 p-1 rounded-lg hover:bg-slate-100 cursor-pointer flex-shrink-0 transition-colors"
                          title="Close"
                        >
                          <X className="w-4 h-4" />
                        </button>
                      ...
                      {/* Emoji Quick Search Bar */}
                      <div className="relative my-2">
                        <Search className="w-3.5 h-3.5 text-slate-400 absolute left-2.5 top-1/2 -translate-y-1/2" />
                        <input
                          type="text"
                          placeholder="ស្វែងរក Emoji (Search)..."
                          value={emojiSearchQuery}
                          onChange={(e) => setEmojiSearchQuery(e.target.value)}
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-7 pr-7 py-1.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#005baa] focus:bg-white transition-all shadow-inner"
                        />
                        {emojiSearchQuery && (
                          <button
                            type="button"
                      ...
                      {/* Category Pills */}
                      {!emojiSearchQuery && (
                        <div className="flex items-center gap-1 py-1 overflow-x-auto no-scrollbar pb-1.5 mb-1 border-b border-slate-100/80">
                      ...
                      {/* Grid */}
                      <div className="grid grid-cols-7 sm:grid-cols-8 gap-1 pt-1 max-h-52 sm:max-h-60 overflow-y-auto pr-0.5 [scrollbar-width:thin]">
                        {filteredEmojis.map((emoji, eIdx) => (
                          <button
                            key={eIdx}
                            type="button"
                            onClick={() => handleInsertEmoji(emoji)}
                            className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl hover:bg-amber-50 active:bg-amber-100 text-lg sm:text-xl flex items-center justify-center cursor-pointer transition-all hover:scale-110 active:scale-95 shadow-2xs border border-transparent hover:border-amber-200"
                          >
                            {emoji}
                          </button>
                        ))}
              ...
              <div ref={gifPickerRef} className="relative flex-shrink-0 hidden sm:block">
                ...
                {/* GIF Picker Popover - rendered via Portal to avoid clipping */}
                {isGifPickerOpen && createPortal(
                  ...
                  >
                    <div 
                      onClick={(e) => e.stopPropagation()}
                      className="fixed z-[10000] w-[460px] max-w-[calc(100vw-24px)] bg-white/98 backdrop-blur-2xl rounded-2xl border border-slate-200 shadow-2xl p-3 select-none ring-1 ring-black/10 font-kantumruy drop-shadow-2xl animate-scale-up"
                      style={{
                        bottom: `${window.innerHeight - (gifPickerRef.current?.getBoundingClientRect()?.top || 0) + 8}px`,
                        left: `${Math.max(8, (gifPickerRef.current?.getBoundingClientRect()?.left || 0) - 80)}px`
                      }}
                    >
                      <div className="flex items-center justify-between pb-2 border-b border-slate-100 gap-2">
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-2 max-h-80 overflow-y-auto pr-1 [scrollbar-width:thin]">
                        {filteredGifs.map((gif) => (
                          <div
                            key={gif.id}
                            onClick={() => handleSendGif(gif)}
                            className="group/gif relative h-32 rounded-2xl overflow-hidden bg-slate-900/10 border border-slate-200 hover:border-indigo-500 shadow-2xs hover:shadow-md cursor-pointer transition-all hover:scale-[1.02]"
                          >
                            <img
                              src={gif.url}
                              alt={gif.titleKm}
